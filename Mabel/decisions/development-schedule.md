...
- [ ] Test 5 (Sunday pancakes) — pending next session.
- [ ] Test 6 (Mrs. Hollis butterscotch) — pending next session.
...
- [ ] `Mabel/Mabel/Models/Memory.swift` — add `var generatedTitle: String?` field. Use `decodeIfPresent` for backward compat.
- [ ] `Mabel/Mabel/Services/OpenAIService.swift` — new `generateMemoryTitle(narrative:)` function. System prompt: *"Generate a 3–5 word evocative title for this memoir memory paragraph. Title only, no quotes, no period. Examples: 'Summer at the Swimming Hole', 'Mother's Sunday Pancakes', 'Mrs. Hollis and the Butterscotch'."* `gpt-4o-mini` is fine here — title generation doesn't need the full model. `max_tokens: 30`, `temperature: 0.7`.
- [ ] `Mabel/Mabel/Services/StoryProcessingService.swift` — after `generateNarrative` succeeds, kick off a follow-up call to `generateMemoryTitle()`. Non-blocking — save the title on the memory when it returns. If it fails, leave `generatedTitle` nil and fall back to the 60-char derivation.
- [ ] `Mabel/Mabel/Components/MemoryCard.swift` `title` computed property — update fallback order: `promptUsed` → `generatedTitle` (NEW) → `derivedTitle(from: narrative)` (existing) → "Written memory" → "Memory N".
- [ ] Cost sanity check: ~30 tokens out × 50 memories per book ≈ negligible (<$0.001/book total).
- [ ] Smoke test: re-record the swimming-hole script, confirm title comes back as ~3 words like "Summer Swimming Hole" or similar.
...
- [ ] Smoke test on real device: HTML script at `/tmp/mabel-smoke-test.html` (Test 1 substantive pancakes script, Test 2 "test test test" verifies language pin returns English, Test 3 silence/background-music verifies hallucination filter). Run after deleting last session's 5 dummy memories from Chapter 1.
