...
- [x] Edge case: chose option (a) — approval is sticky. Implemented in `StoryProcessingService.processMemory` and `processTypedMemory`: chapter-narrative regeneration is gated on `!updatedChapter.isApproved`. Without this guard, every post-approval recording would call `updateNarrative()`, which also clears `isApproved`. The 6th+ memory now lands in the list with a generated memory-narrative + title, but the published chapter narrative stays frozen. (2026-05-20)
- [ ] Smoke test: in a 5/5 approved chapter, tap mic, record a substantive 6th memory. Confirm: it appears in the list, gets a narrative, the chapter approval is NOT revoked. **Pending on-device verification.**
