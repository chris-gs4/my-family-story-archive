...
- [ ] Revisit `capturedToast` in `RecordingSetupView.swift`. Consider: (a) different bg color (mabelPrimaryLight reads flat — try mabelBackground with stronger border + shadow, or mabelPrimary with white text/icon), (b) icon shape (sealed checkmark, document-with-check, sparkle — pull from SF Symbols + the existing checkmark.seal.fill the approval indicator uses for consistency), (c) shadow + corner radius treatment, (d) entrance/exit easing — current `easeOut(0.3)` may feel mechanical; try `MabelAnimation` spring tokens, (e) typography weight (helper() is regular Comfortaa — try semibold or the badge() font for "Memory captured"). Easiest path: spin up `/design-uplift` with the current toast as input and generate 3 competing concepts.
