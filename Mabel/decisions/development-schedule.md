...
- **2026-05-18 — Phase 1.6 (AI memory titles) implemented and committed (f37ff72), smoke-test partially run.** Mrs. Hollis test blocked by the Phase 1.0 false-positive above (the narrative never gets generated, so title gen never fires). After the refusal-clause patch + redeploy, retry the same memory: should land green/healthy with narrative, then within a few seconds the title should swap from the 60-char derivation to a 3–5 word AI title like "Mrs. Hollis and the Butterscotch". Second test (swimming hole) still queued.
